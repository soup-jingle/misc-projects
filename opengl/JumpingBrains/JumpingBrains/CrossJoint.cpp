...
CrossJoint::CrossJoint(float sphRad, float sphCent[], float sphClr[], float sphSca[],
...
void CrossJoint::initialize(const float sphRad, const float sphCent[], const float sphClr[], const float sphSca[],
...
// Update CrossJoint's roll and yaw, altering increment values if necessary.
// VelocityFactor specifies the rate at which it should change its roll and
// yaw relative to its base roll and yaw increments. VelocityFactor equal to
// 1.0 would be no change, < 1.0 would be slower, > 1.0 would be faster
void CrossJoint::update()
{
	CurrentRoll += VelocityFactor * RollIncrement;
	...
	CurrentYaw += VelocityFactor * YawIncrement;
