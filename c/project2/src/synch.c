...
static Cond conds[MAX_CONDS]; //All conditional variables
...
LockHandleAcquire(lock_t lock)
{
  //Your stuff goes here
	...
	l = &locks[lock]; // Get Lock from lock handle
	if (l->inuse == 1) {
		if(pid == l->pid) { // Check current process owns lock
			RestoreIntrs(intrval);
			return 0;
		}
		else { // Check other process owns lock
			SemWait(&(l->mutex));
		}
	}
	else { // Lock not owned
...
CondHandleWait(cond_t cond)
{
  // Your stuff goes here
	...
	if(locks[lock].pid != pid) { // Calling process does not own lock
		RestoreIntrs(intrval);
		return 1;
	} 
	LockHandleRelease(lock);
	RestoreIntrs(intrval);
	SemWait(&(locks[lock].mutex));
	LockHandleAcquire(lock);
}
...
CondHandleSignal(cond_t cond)
{
  // Your stuff goes here
	...
	c = &conds[cond];
	l = &locks[c->lock];
	if(l->pid != pid) // Calling process does not own lock
		return 1;
