(def the-matrix ((fn [list] 
                   (into [] (for [l list] 
                              (into [] l)))) matrix-as-list))
...
(def max-matrix-product 
  (apply max (for [i (range 17)
                   j (range 17)
                   :let [t ((the-matrix i) j)
                         a ((the-matrix (inc i)) j) 
                         b ((the-matrix (+ i 2)) j) 
                         c ((the-matrix (+ i 3)) j)
                         d ((the-matrix (inc i)) (inc j)) 
                         e ((the-matrix (+ i 2)) (+ j 2)) 
                         f ((the-matrix (+ i 3)) (+ j 3))
                         g ((the-matrix i) (inc j)) 
                         h ((the-matrix i) (+ j 2))
                         i ((the-matrix i) (+ j 3))]] 
               (max (* t a b c) (* t d e f) (* t g h i)))))
...
(def max-matrix-product
  (apply max 
         (for [i (range 0 17)
               j (range 3 20)
               :let [t ((the-matrix i) j)
                     a ((the-matrix (inc i)) (dec j))
                     b ((the-matrix (+ i 2)) (- j 2))
                     c ((the-matrix (+ i 3)) (- j 3))]] 
           (* t a b c)))))
